        ...
        Pass
        {
			...
			float3 clipAABB(float3 nowColor, float3 preColor, float2 screenPosition)
			{
				...
				for(int i=-1;i<=1;++i)
				{
					...
					{
						float2 newUV = screenPosition + deltaRes * float2(i, j);
						float3 C = RGB2YCoCgR(ToneMap(tex2D(currentColor, newUV).rgb));
						m1 += C;
						m2 += C * C;
					}
				}
            ...
            {
				float3 nowColor = tex2D(currentColor, i.uv).rgb;
				...
				float2 velocity = tex2D(velocityTexture, closestUV).rg;

				float2 offsetUV = clamp(i.uv - velocity, 0, 1);
				float3 preColor = tex2D(_MainTex, offsetUV).rgb;
